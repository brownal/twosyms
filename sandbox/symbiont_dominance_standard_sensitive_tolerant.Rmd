...
```{r, cache=TRUE}
# For calculating C vs. N limitation
...
plot(NULL, xaxt='n',yaxt='n',bty='n',ylab='',xlab='', xlim=0:1, ylim=0:1)
legend("topleft", c("Sens. N/C-lim\n", "Tol. N/C-lim\n", "Both N/C-lim"), col=c("blue", "red", "purple"), pch=19)

plot(NULL, xaxt='n',yaxt='n',bty='n',ylab='',xlab='', xlim=0:1, ylim=0:1, main="jHG N/C lim.\n+ = N-lim, - = C-lim")
...
for(i in 1:39) {
      ...
      s_cn_lim_i <- mapply(function(H, S, jX, jN, rhoC, rNH, yC, nNX, nNH, Hgm) {
            limitation(carbonInput1S(yC, rhoC, S, H, jX),
                  nitrogenInput(jN, nNX, jX, rNH, nNH), Hgm)
            },
      H=s_run_i$H, S=s_run_i$S, jX=s_run_i$jX,
      jN=s_run_i$jN, rhoC=s_run_i$rhoC, rNH=s_run_i$rNH,
      MoreArgs=list(yC=s_pars_i$yC, nNX=s_pars_i$nNX, nNH=s_pars_i$nNH, Hgm=s_pars_i$Hgm))

      t_cn_lim_i <- mapply(function(H, S, jX, jN, rhoC, rNH, yC, nNX, nNH, Hgm) {
            limitation(carbonInput1S(yC, rhoC, S, H, jX),
                  nitrogenInput(jN, nNX, jX, rNH, nNH), Hgm)
            },
      H=t_run_i$H, S=t_run_i$S, jX=t_run_i$jX,
      jN=t_run_i$jN, rhoC=t_run_i$rhoC, rNH=t_run_i$rNH,
      MoreArgs=list(yC=t_pars_i$yC, nNX=t_pars_i$nNX, nNH=t_pars_i$nNH, Hgm=t_pars_i$Hgm))

      both_cn_lim_i <- mapply(function(H, S.1, S.2, jX, jN, rhoC.1, rhoC.2, rNH, yC, nNX, nNH, Hgm) {
            limitation(carbonInput2S(yC, rhoC.1, rhoC.2, S.1, S.2, H, jX),
                  nitrogenInput(jN, nNX, jX, rNH, nNH), Hgm)
            },
      H=both_run_i$H, S.1=both_run_i$S.1, S.2=both_run_i$S.2, jX=both_run_i$jX,
      jN=both_run_i$jN, rhoC.1=both_run_i$rhoC.1, rhoC.2=both_run_i$rhoC.2, rNH=both_run_i$rNH,
      MoreArgs=list(yC=both_pars_i$yC, nNX=both_pars_i$nNX, nNH=both_pars_i$nNH, Hgm=both_pars_i$Hgm))


      plot(time[-1], s_run_i$dH.Hdt[-1], pch=20,
            main=paste0("Sim. ", i, ": sens."),
            xlab="time", ylab="", col=ifelse(s_run_i$dH.Hdt[-1] > 0, "green", "black"),
            ylim=c(min(s_run_i$dH.Hdt, s_run_i$S/s_run_i$H), max(s_run_i$dH.Hdt, s_run_i$S/s_run_i$H)))
      lines(time[-1], s_run_i$S[-1]/s_run_i$H[-1], col='blue')

      plot(time[-1], t_run_i$dH.Hdt[-1], pch=20,
            main=paste0("Sim. ", i, ": tol."),
            xlab="time", ylab="", col=ifelse(t_run_i$dH.Hdt[-1] > 0, "green", "black"),
            ylim=c(min(t_run_i$dH.Hdt, t_run_i$S/t_run_i$H), max(t_run_i$dH.Hdt, t_run_i$S/t_run_i$H)))
      lines(time[-1], t_run_i$S[-1]/t_run_i$H[-1], col='red')

      plot(time[-1], both_run_i$dH.Hdt[-1],  pch=20,
            main=paste0("Sim. ", i, ": both"),
            xlab="time", ylab="", col=ifelse(both_run_i$dH.Hdt[-1] > 0, "green", "black"),
            ylim=c(min(both_run_i$dH.Hdt, both_run_i$S/both_run_i$H), max(both_run_i$dH.Hdt, both_run_i$S/both_run_i$H)))
      lines(time[-1], both_run_i$S.1[-1]/both_run_i$H[-1], col='blue')
      lines(time[-1], both_run_i$S.2[-1]/both_run_i$H[-1], col='red')

      plot(time[-1], s_cn_lim_i[-1], type="l", col="blue",
            main=paste0("Sim. ", i, ": N/C lim"), xlab="time", ylab="",
            ylim=c(min(s_cn_lim_i, t_cn_lim_i, both_cn_lim_i), max(s_cn_lim_i, t_cn_lim_i, both_cn_lim_i)))
      lines(time[-1], t_cn_lim_i[-1], col="red")
      lines(time[-1], both_cn_lim_i[-1], col="purple")
}
...
```{r, cache=TRUE}
# selected_sims <- healthy_sims[!(healthy_sims$survival_both) & (healthy_sims$survival_tol & healthy_sims$survival_sen), ]
# par(mfrow=c(6, 4), mar=c(5.1, 2, 1, 1))
#
# for(i in 12:22) {
#       sim_i <- selected_sims[i,]
#       env_i <- coRal::init_env(time=time,
...
```{r}
sum(increased_sh_sims$survival_sen & increased_sh_sims$survival_tol & increased_sh_sims$survival_both) == nrow(increased_sh_sims)
```



### 1h. Do hosts with a single symbiont survive when the initial symbiont:host ratio is 0.05?
```{r, cache=TRUE}
...
decreased_sh <- foreach(i=1:nrow(selected_sims), .combine='rbind', .packages='dplyr') %dopar% {
      sim_i <- selected_sims[i,]
      env_i <- coRal::init_env(time=time,
            L=c(as.numeric(sim_i[1, "L"]), as.numeric(sim_i[1, "L"]), 0),
            N=c(as.numeric(sim_i[1, "N"]), as.numeric(sim_i[1, "N"]), 0),
      ...
      both_pars_i$kROS[2] <- 250
      both_pars_i$jSGm[2] <- 0.15
      both_pars_i$initS <- c(0.025, 0.025)
...
# store results in dataframe
decreased_sh_sims <- data.frame(decreased_sh, row.names=NULL)
```

```{r}
# set the columns to their proper types
decreased_sh_sims <- transform(decreased_sh_sims,
      L=as.numeric(L), N=as.numeric(N), X=as.numeric(X), kCO2=as.numeric(kCO2),
      converge_sen=as.logical(converge_sen), converge_tol=as.logical(converge_tol), converge_both=as.logical(converge_both),
      survival_sen=as.logical(survival_sen), survival_tol=as.logical(survival_tol), survival_both=as.logical(survival_both),
      tol_per_sen=as.numeric(tol_per_sen),
...
#### 1hi.What percent of hosts with only a sensitive symbiont now survive?

```{r}
print(paste0(round(sum(decreased_sh_sims$survival_sen)/nrow(decreased_sh_sims)*100, 2), "%"))
```

#### 1hi. What percent of hosts with only a tolerant symbiont now survive?

```{r}
print(paste0(round(sum(decreased_sh_sims$survival_tol)/ nrow(decreased_sh_sims)*100, 2), "%"))
```

#### 1hii. Whaaat?? Why do the tolerant symbionts all survive? Plot the time series

```{r, cache=TRUE}
selected_sims <- decreased_sh_sims
par(mfrow=c(6, 6), mar=c(5.1, 2, 1, 1))
...
### 1i. What happens if we shorten the time step?
```{r, cache=TRUE}

time_ts <-seq(1, 600, 0.01)
...
decreased_ts <- foreach(i=1:nrow(selected_sims), .combine='rbind', .packages='dplyr') %dopar% {
      sim_i <- selected_sims[i,]
      env_i <- coRal::init_env(time=time_ts,
            L=c(as.numeric(sim_i[1, "L"]), as.numeric(sim_i[1, "L"]), 0),
            N=c(as.numeric(sim_i[1, "N"]), as.numeric(sim_i[1, "N"]), 0),
            X=c(as.numeric(sim_i[1, "X"]), as.numeric(sim_i[1, "X"]), 0))
      ...
      run_sen <- coRal::run_coral(time=time_ts, env=env_i, pars=s_pars_i)
      run_tol <- coRal::run_coral(time=time_ts, env=env_i, pars=t_pars_i)
      run_both <- coRal::run_coral(time=time_ts, env=env_i, pars=both_pars_i)



      # 1. Convergence: have the host and symbiont growth rates stabilized? (Changed by no more than 10^-6 between the last and penultimate time steps)
      # 1a. Simulation with sensitive symbiont alone
      converge_sen_H <- (abs(run_sen$dH.Hdt[length(time_ts)] - run_sen$dH.Hdt[length(time_ts)-1]) <= 1e-6)
      converge_sen_S <- (abs(run_sen$dS.Sdt[length(time_ts)] - run_sen$dS.Sdt[length(time_ts)-1]) <= 1e-6)
      converge_sen <- converge_sen_H & converge_sen_S

      # 1b. Simulation with tolerant symbiont alone
      converge_tol_H <- (abs(run_tol$dH.Hdt[length(time_ts)] - run_tol$dH.Hdt[length(time_ts)-1]) <= 1e-6)
      converge_tol_S <- (abs(run_tol$dS.Sdt[length(time_ts)] - run_tol$dS.Sdt[length(time_ts)-1]) <= 1e-6)
      converge_tol <- converge_tol_H & converge_tol_S

      # 1c. Simulation with both symbionts
      converge_both_H <- (abs(run_both$dH.Hdt[length(time_ts)] - run_both$dH.Hdt[length(time_ts)-1]) <= 1e-6)
      converge_both_S1 <- (abs(run_both$dS.Sdt.1[length(time_ts)] - run_both$dS.Sdt.1[length(time_ts)-1]) <= 1e-6)
      converge_both_S2 <- (abs(run_both$dS.Sdt.2[length(time_ts)] - run_both$dS.Sdt.2[length(time_ts)-1]) <= 1e-6)
      converge_both <- converge_both_H & (converge_both_S1 & converge_both_S2)

      # 2. Survival: does the coral have positive growth at the end of the simulation?
      survival_sen <- run_sen$dH.Hdt[length(time_ts)] > 0
      survival_tol <- run_tol$dH.Hdt[length(time_ts)] > 0
      survival_both <- run_both$dH.Hdt[length(time_ts)] > 0

      # 3. Dominance: when both symbionts are present, what is the sensitive:tolerant symbiont ratio?
      tol_per_sen = run_both$S.2[length(time_ts)]/run_both$S.1[length(time_ts)]

      # 4. Symbiont: host ratio
      sh_ratio_sen = run_sen$S[length(time_ts)]/run_sen$H[length(time_ts)]
      sh_ratio_tol = run_tol$S[length(time_ts)]/run_tol$H[length(time_ts)]
      sh_ratio_both = (run_both$S.1[length(time_ts)] + run_both$S.2[length(time_ts)])/run_both$H[length(time_ts)]

      # 5. Host growth rate
      host_growth_sen = run_sen$dH.Hdt[length(time_ts)]
      host_growth_tol = run_tol$dH.Hdt[length(time_ts)]
      host_growth_both = run_both$dH.Hdt[length(time_ts)]
...
# store results in dataframe
decreased_ts_sims <- data.frame(decreased_ts, row.names=NULL)
```

```{r}
# set the columns to their proper types
decreased_ts_sims <- transform(decreased_ts_sims,
      L=as.numeric(L), N=as.numeric(N), X=as.numeric(X), kCO2=as.numeric(kCO2),
      converge_sen=as.logical(converge_sen), converge_tol=as.logical(converge_tol), converge_both=as.logical(converge_both),
      survival_sen=as.logical(survival_sen), survival_tol=as.logical(survival_tol), survival_both=as.logical(survival_both),
      tol_per_sen=as.numeric(tol_per_sen),
...
#### 1i ii.What percent of hosts with only a sensitive symbiont now survive?

```{r}
print(paste0(round(sum(decreased_ts_sims$survival_sen)/nrow(decreased_ts_sims)*100, 2), "%"))
```

#### 1i iii. What percent of hosts with only a tolerant symbiont now survive?

```{r}
print(paste0(round(sum(decreased_ts_sims$survival_tol)/ nrow(decreased_ts_sims)*100, 2), "%"))
```


#### 1i iv. What percent of hosts with both symbionts now survive?

```{r}
print(paste0(round(sum(decreased_ts_sims$survival_both)/ nrow(decreased_ts_sims)*100, 2), "%"))
```
