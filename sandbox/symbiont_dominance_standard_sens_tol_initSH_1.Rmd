...
Find subclusters of the outcomes?

```{r, cache=TRUE}
# Find subclusters of no survival and tolerant-dominant cases
NS_clust <- find.clusters(dapc_sims[dapc_sims$dominant_symb=="No_survival", c("L", "X", "N", "kCO2")], n.clust=3, n.pca=3)$grp

T_clust <- find.clusters(dapc_sims[dapc_sims$dominant_symb=="Tolerant", c("L", "X", "N", "kCO2")], n.clust=3, n.pca=3)$grp

# plot the subclusters
print("No survival subclusters")
plot(dapc_sims[dapc_sims$dominant_symb=="No_survival", c("L", "X", "N", "kCO2")], col="black", pch=c(0, 7, 15)[NS_clust])

print("Tolerant symb. dominates hosts subclusters")
plot(dapc_sims[dapc_sims$dominant_symb=="Tolerant", c("L", "X", "N", "kCO2")], col="#FFB000", pch=c(1, 10, 16)[T_clust])


# Make a new dataframe with a column (new_clust) for the subclusters
dapc_sims_clust <- dapc_sims
dapc_sims_clust$new_clust <- as.character(dapc_sims$dominant_symb)

dapc_sims_clust[dapc_sims_clust$dominant_symb=="No_survival", "new_clust"] <- paste0(as.character(dapc_sims_clust[dapc_sims_clust$dominant_symb=="No_survival", "dominant_symb"]), " - ", NS_clust)
dapc_sims_clust[dapc_sims_clust$dominant_symb=="Tolerant", "new_clust"] <- paste0(as.character(dapc_sims_clust[dapc_sims_clust$dominant_symb=="Tolerant", "dominant_symb"]), " - ", T_clust)

dapc_sims_clust$new_clust <- as.factor(dapc_sims_clust$new_clust)

# Plots to check that the new column correctly labels things
#plot(dapc_sims[dapc_sims$dominant_symb=="Tolerant", c("L", "X", "N", "kCO2")], col=dapc_sims_clust[dapc_sims_clust$dominant_symb=="Tolerant", "new_clust"])
#plot(dapc_sims[dapc_sims$dominant_symb=="No_survival", c("L", "X", "N", "kCO2")], col=dapc_sims_clust[dapc_sims_clust$dominant_symb=="No_survival", "new_clust"])

mydapc_clust <- dapc(dapc_sims_clust[, c("L", "N", "X", "kCO2")], grp=dapc_sims_clust$new_clust, var.contrib=TRUE, var.loadings=TRUE, scale = TRUE, n.pca = 4, n.da = 3)

```

Plot DAPC with subgroups

```{r}

scatter(mydapc_clust, cell = 0, cstar = 0, mstree = FALSE, lwd = 2, lty = 2, axesell=TRUE, scree.da=FALSE, inset.pca=TRUE,
      col=c("#000000", "#000000", "#648FFF", "#FFB000", "#FFB000"), pch=c(0, 7, 2, 1, 16))
arrows(x0=0, y0=0, x1=mydapc_clust$var.load[1,1], y1=mydapc_clust$var.load[1,2])
text(x=mydapc_clust$var.load[1,1]/2, y=mydapc_clust$var.load[1,2]/2, labels="Light", pos=3)
arrows(x0=0, y0=0, x1=mydapc_clust$var.load[2,1], y1=mydapc_clust$var.load[2,2])
text(x=mydapc_clust$var.load[2,1]/1, y=mydapc_clust$var.load[2,2]/2, labels="Nitrogen", pos=4)
arrows(x0=0, y0=0, x1=mydapc_clust$var.load[3,1], y1=mydapc_clust$var.load[3,2])
text(x=mydapc_clust$var.load[3,1]/2, y=mydapc_clust$var.load[3,2]/2, labels="Food", pos=4)
arrows(x0=0, y0=0, x1=mydapc_clust$var.load[4,1], y1=mydapc_clust$var.load[4,2])
text(x=mydapc_clust$var.load[4,1]/2, y=mydapc_clust$var.load[4,2]/2, labels="KCO2", pos=4)
summary(mydapc_clust)


```


Try DAPC with two subgroups for No survival and Tolerant-dominated host

```{r, cache=TRUE}

# Find subclusters of no survival and tolerant-dominant cases
NS_clust <- find.clusters(dapc_sims[dapc_sims$dominant_symb=="No_survival", c("L", "X", "N", "kCO2")], n.clust=2, n.pca=3)$grp

T_clust <- find.clusters(dapc_sims[dapc_sims$dominant_symb=="Tolerant", c("L", "X", "N", "kCO2")], n.clust=2, n.pca=3)$grp
...
No survival subclusters

```{r}
# plot the subclusters
par(mfrow=c(3,3))
plot(jitter(dapc_sims[dapc_sims$dominant_symb=="No_survival", "L"]), jitter(dapc_sims[dapc_sims$dominant_symb=="No_survival", "N"]) , col="black", pch=c(0, 15)[NS_clust], xlab="L", ylab="N")
plot(jitter(dapc_sims[dapc_sims$dominant_symb=="No_survival", "L"]), jitter(dapc_sims[dapc_sims$dominant_symb=="No_survival", "X"]) , col="black", pch=c(0, 15)[NS_clust], xlab="L", ylab="X")
plot(jitter(dapc_sims[dapc_sims$dominant_symb=="No_survival", "L"]), jitter(dapc_sims[dapc_sims$dominant_symb=="No_survival", "kCO2"]) , col="black", pch=c(0, 15)[NS_clust], xlab="L", ylab="kCO2")
plot(jitter(dapc_sims[dapc_sims$dominant_symb=="No_survival", "N"]), jitter(dapc_sims[dapc_sims$dominant_symb=="No_survival", "X"]) , col="black", pch=c(0, 15)[NS_clust], xlab="N", ylab="X")
plot(jitter(dapc_sims[dapc_sims$dominant_symb=="No_survival", "N"]), jitter(dapc_sims[dapc_sims$dominant_symb=="No_survival", "kCO2"]) , col="black", pch=c(0, 15)[NS_clust], xlab="N", ylab="kCO2")
plot(jitter(dapc_sims[dapc_sims$dominant_symb=="No_survival", "X"]), jitter(dapc_sims[dapc_sims$dominant_symb=="No_survival", "kCO2"]) , col="black", pch=c(0, 15)[NS_clust], xlab="X", ylab="kCO2")

```

Tolerant symb. dominates hosts subclusters

```{r}

par(mfrow=c(3,3))
plot(jitter(dapc_sims[dapc_sims$dominant_symb=="Tolerant", "L"]), jitter(dapc_sims[dapc_sims$dominant_symb=="Tolerant", "N"]) , col=c("#0000FF", "#FFB000")[T_clust], pch=c(16, 16)[T_clust], xlab="L", ylab="N")
plot(jitter(dapc_sims[dapc_sims$dominant_symb=="Tolerant", "L"]), jitter(dapc_sims[dapc_sims$dominant_symb=="Tolerant", "X"]) , col=c("#0000FF", "#FFB000")[T_clust], pch=c(16, 16)[T_clust], xlab="L", ylab="X")
plot(jitter(dapc_sims[dapc_sims$dominant_symb=="Tolerant", "L"]), jitter(dapc_sims[dapc_sims$dominant_symb=="Tolerant", "kCO2"]) , col=c("#0000FF", "#FFB000")[T_clust], pch=c(16, 16)[T_clust], xlab="L", ylab="kCO2")
plot(jitter(dapc_sims[dapc_sims$dominant_symb=="Tolerant", "N"]), jitter(dapc_sims[dapc_sims$dominant_symb=="Tolerant", "X"]) , col=c("#0000FF", "#FFB000")[T_clust], pch=c(16, 16)[T_clust], xlab="N", ylab="X")
plot(jitter(dapc_sims[dapc_sims$dominant_symb=="Tolerant", "N"]), jitter(dapc_sims[dapc_sims$dominant_symb=="Tolerant", "kCO2"]) ,col=c("#0000FF", "#FFB000")[T_clust], pch=c(16, 16)[T_clust], xlab="N", ylab="kCO2")
plot(jitter(dapc_sims[dapc_sims$dominant_symb=="Tolerant", "X"]), jitter(dapc_sims[dapc_sims$dominant_symb=="Tolerant", "kCO2"]) , col=c("#0000FF", "#FFB000")[T_clust], pch=c(16, 16)[T_clust], xlab="X", ylab="kCO2")
...
```{r}
scatter(mydapc_thinned, cell = 0, cstar = 0, mstree = FALSE, lwd = 2, lty = 2, axesell=TRUE, scree.da=FALSE, inset.pca=TRUE,
      col=c("#000000", "#648FFF", "#FFB000"), pch=c(0, 2, 16))
arrows(x0=0, y0=0, x1=mydapc_thinned$var.load[1,1], y1=mydapc_thinned$var.load[1,2])
text(x=mydapc_thinned$var.load[1,1]/2, y=mydapc_thinned$var.load[1,2]/2, labels="Light", pos=3)
arrows(x0=0, y0=0, x1=mydapc_thinned$var.load[2,1], y1=mydapc_thinned$var.load[2,2])
text(x=mydapc_thinned$var.load[2,1]/1, y=mydapc_thinned$var.load[2,2]/2, labels="Nitrogen", pos=4)
arrows(x0=0, y0=0, x1=mydapc_thinned$var.load[3,1], y1=mydapc_thinned$var.load[3,2])
text(x=mydapc_thinned$var.load[3,1]/2, y=mydapc_thinned$var.load[3,2]/2, labels="Food", pos=4)
arrows(x0=0, y0=0, x1=mydapc_thinned$var.load[4,1], y1=mydapc_thinned$var.load[4,2])
text(x=mydapc_thinned$var.load[4,1]/2, y=mydapc_thinned$var.load[4,2]/2, labels="KCO2", pos=4)
