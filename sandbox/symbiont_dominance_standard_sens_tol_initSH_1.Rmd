...
for (i in 1:length(unique(healthy_sims$N))) {
      for (j in 1:length(unique(healthy_sims$L))) {
            tol_per_sen <- healthy_sims[(healthy_sims$N == unique(healthy_sims$N)[[i]]) & (healthy_sims$L == unique(healthy_sims$L)[[j]]) &
                  (healthy_sims$X == 2e-7) & (healthy_sims$kCO2 == 22) & healthy_sims$survival_both, "tol_per_sen"]
...
## Plot dominance for a few food, kCO2 combinations

```{r}
par(mfrow=c(3, 3), mar=c(5.1, 4.1, 4.1, 6.1))

for (x in c(8e-8, 2e-7, 3.2e-7)) {
      for (kCO2 in c(10, 18, 26)) {
            ...
            for (i in 1:length(unique(healthy_sims$N))) {
                  for (j in 1:length(unique(healthy_sims$L))) {
                        tol_per_sen <- healthy_sims[(healthy_sims$N == unique(healthy_sims$N)[[i]]) & (healthy_sims$L == unique(healthy_sims$L)[[j]]) &
                              (healthy_sims$X == x) & (healthy_sims$kCO2 == kCO2) & healthy_sims$survival_both, "tol_per_sen"]
