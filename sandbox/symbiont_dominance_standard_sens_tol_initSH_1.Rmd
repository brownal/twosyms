...
lightPts <- seq(min(healthy_sims$L), max(healthy_sims$L), length.out=300)
nitrogenPts <- seq(min(healthy_sims$N), max(healthy_sims$N), length.out=300)
#foodPts <- seq(min(healthy_sims$X), max(healthy_sims$X), length.out=300)

par(mfrow=c(4, 3), mar=c(5.1, 4.1, 4.1, 6.1))
palette(c("black", "#648FFF", "#FFB000", "green"))

for (x in unique(healthy_sims$X)) {
      combinedPts <- expand.grid(L=lightPts, N=nitrogenPts, X=x)
      ...
      plot(healthy_sims[healthy_sims$X==x, "L"], healthy_sims[healthy_sims$X==x, "N"], col = healthy_sims[healthy_sims$X==x, "dominant_symb"],
            xlab="Light", ylab="Nitrogen", main=paste0("Food = ", x))
...
for (N in unique(healthy_sims$N)) {
      ...
      plot(healthy_sims[healthy_sims$N==N, "L"], healthy_sims[healthy_sims$N==N, "X"], col = healthy_sims[healthy_sims$N==N, "dominant_symb"],
...
```{r}
mydapc <- dapc(healthy_sims[, c("L", "N", "X")], grp=healthy_sims$dominant_symb, var.contrib=TRUE, scale = TRUE, n.pca = 3, n.da = 2)
scatter(mydapc, cell = 0, pch = 18:23, cstar = 0, mstree = TRUE, lwd = 2, lty = 2, axesell=TRUE)
summary(mydapc)
```
