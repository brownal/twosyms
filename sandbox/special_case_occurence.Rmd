...
Light_data = data.frame(bins=seq(0, 40, 5), rescue=NA, S_lives=NA, T_dies=NA, T_dominates=NA)

for (i in 1:length(Light_data$bins)-1) {
    Light_data$rescue[i] <- sum(with(rescue_jCPm_1, L >= Light_data$bins[i] & L < Light_data$bins[i+1] & rescue==TRUE))/
    sum(with(rescue_jCPm_1, L >= Light_data$bins[i] & L < Light_data$bins[i+1]))

    Light_data$S_lives[i] <- sum(with(rescue_jCPm_1, L >= Light_data$bins[i] & L < Light_data$bins[i+1] & reason=="S_lives"))/
    sum(with(rescue_jCPm_1, L >= Light_data$bins[i] & L < Light_data$bins[i+1]))

    Light_data$T_dies[i] <- sum(with(rescue_jCPm_1, L >= Light_data$bins[i] & L < Light_data$bins[i+1] & reason=="T_dies"))/
    sum(with(rescue_jCPm_1, L >= Light_data$bins[i] & L < Light_data$bins[i+1]))

    Light_data$T_dominates[i] <- sum(with(rescue_jCPm_1, L >= Light_data$bins[i] & L < Light_data$bins[i+1] & reason=="T_dominates"))/
  sum(with(rescue_jCPm_1, L >= Light_data$bins[i] & L < Light_data$bins[i+1]))
}

barplot(t(as.matrix(Light_data[1:(nrow(Light_data)-1),-1])), names.arg=Light_data$bins[-nrow(Light_data)], col=c("blue", "pink", "green", "black"),
      legend.text=c("rescue", "S. lives", "T. dies", "T. dominates"),
      xlab="Light", main="Light", ylab="Fraction of tested values")


DIN_data = data.frame(bins=seq(0, 4e-6, 5e-7), rescue=NA, S_lives=NA, T_dies=NA, T_dominates=NA)

for (i in 1:length(DIN_data$bins)-1) {
    DIN_data$rescue[i] <- sum(with(rescue_jCPm_1, N >= DIN_data$bins[i] & N < DIN_data$bins[i+1] & rescue==TRUE))/
    sum(with(rescue_jCPm_1, N >= DIN_data$bins[i] & N < DIN_data$bins[i+1]))

    DIN_data$S_lives[i] <- sum(with(rescue_jCPm_1, N >= DIN_data$bins[i] & N < DIN_data$bins[i+1] & reason=="S_lives"))/
    sum(with(rescue_jCPm_1, N >= DIN_data$bins[i] & N < DIN_data$bins[i+1]))

    DIN_data$T_dies[i] <- sum(with(rescue_jCPm_1, N >= DIN_data$bins[i] & N < DIN_data$bins[i+1] & reason=="T_dies"))/
    sum(with(rescue_jCPm_1, N >= DIN_data$bins[i] & N < DIN_data$bins[i+1]))

    DIN_data$T_dominates[i] <- sum(with(rescue_jCPm_1, N >= DIN_data$bins[i] & N < DIN_data$bins[i+1] & reason=="T_dominates"))/
    sum(with(rescue_jCPm_1, N >= DIN_data$bins[i] & N < DIN_data$bins[i+1]))
}

barplot(t(as.matrix(DIN_data[1:(nrow(DIN_data)-1),-1])), names.arg=DIN_data$bins[-nrow(DIN_data)], col=c("blue", "pink", "green", "black"),
      xlab="DIN", main="DIN", ylab="Fraction of tested values")



Food_data = data.frame(bins=seq(0, 4e-7, 5e-8), rescue=NA, S_lives=NA, T_dies=NA, T_dominates=NA)

for (i in 1:length(Food_data$bins)-1) {
    Food_data$rescue[i] <- sum(with(rescue_jCPm_1, X >= Food_data$bins[i] & X < Food_data$bins[i+1] & rescue==TRUE))/
    sum(with(rescue_jCPm_1, X >= Food_data$bins[i] & X < Food_data$bins[i+1]))

    Food_data$S_lives[i] <- sum(with(rescue_jCPm_1, X >= Food_data$bins[i] & X < Food_data$bins[i+1] & reason=="S_lives"))/
    sum(with(rescue_jCPm_1, X >= Food_data$bins[i] & X < Food_data$bins[i+1]))

    Food_data$T_dies[i] <- sum(with(rescue_jCPm_1, X >= Food_data$bins[i] & X < Food_data$bins[i+1] & reason=="T_dies"))/
    sum(with(rescue_jCPm_1, X >= Food_data$bins[i] & X < Food_data$bins[i+1]))

    Food_data$T_dominates[i] <- sum(with(rescue_jCPm_1, X >= Food_data$bins[i] & X < Food_data$bins[i+1] & reason=="T_dominates"))/
    sum(with(rescue_jCPm_1, X >= Food_data$bins[i] & X < Food_data$bins[i+1]))
}

barplot(t(as.matrix(Food_data[1:(nrow(Food_data)-1),-1])), names.arg=Food_data$bins[-nrow(Food_data)], col=c("blue", "pink", "green", "black"),
      xlab="Food", main="Food", ylab="Fraction of tested values")


kCO2_data = data.frame(bins=seq(10, 50, 5), rescue=NA, S_lives=NA, T_dies=NA, T_dominates=NA)

for (i in 1:length(kCO2_data$bins)-1) {
    kCO2_data$rescue[i] <- sum(with(rescue_jCPm_1, kCO2 >= kCO2_data$bins[i] & X < kCO2_data$bins[i+1] & rescue==TRUE))/
    sum(with(rescue_jCPm_1, kCO2 >= kCO2_data$bins[i] & X < kCO2_data$bins[i+1]))

    kCO2_data$S_lives[i] <- sum(with(rescue_jCPm_1, kCO2 >= kCO2_data$bins[i] & X < kCO2_data$bins[i+1] & reason=="S_lives"))/
    sum(with(rescue_jCPm_1, kCO2 >= kCO2_data$bins[i] & X < kCO2_data$bins[i+1]))

    kCO2_data$T_dies[i] <- sum(with(rescue_jCPm_1, kCO2 >= kCO2_data$bins[i] & X < kCO2_data$bins[i+1] & reason=="T_dies"))/
    sum(with(rescue_jCPm_1, kCO2 >= kCO2_data$bins[i] & X < kCO2_data$bins[i+1]))

    kCO2_data$T_dominates[i] <- sum(with(rescue_jCPm_1, kCO2 >= kCO2_data$bins[i] & X < kCO2_data$bins[i+1] & reason=="T_dominates"))/
    sum(with(rescue_jCPm_1, kCO2 >= kCO2_data$bins[i] & X < kCO2_data$bins[i+1]))
}

barplot(t(as.matrix(kCO2_data[1:(nrow(kCO2_data)-1),-1])), names.arg=kCO2_data$bins[-nrow(kCO2_data)], col=c("blue", "pink", "green", "black"),
      xlab="kCO2", main="kCO2", ylab="Fraction of tested values")
...
pars2$kROS[2] <- 250
pars2$jSGm[2] <- 0.15
pars2$initS[1:2] <- c(0.5e-3, 0.5e-3)
