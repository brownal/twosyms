...
```{r plot-recovery, cache=TRUE, dependson=c("sim-recovery"), fig.width=10, fig.height=5}
  ...
  # Light
  stackedHist(data=list(sim_recovery$L[(sim_recovery$outcome == "recovers-S")],
    ...
    breaks=seq(5, 60, 2.5), xlim=c(5, 60), ylim=c(0, 6), colors=c(colSen, colTol, colRescue, colNS),
  ...
  points(sim_recovery$L[(sim_recovery$outcome == "recovers-T")],
    rep(6, sum(sim_recovery$outcome == "recovers-T")),col=colTol, pch=pchTol)

  points(sim_recovery$L[(sim_recovery$outcome == "recovers-S-ip-T")],
    rep(5.5, sum(sim_recovery$outcome == "recovers-S-ip-T")),col=colRescue, pch=pchRescue)


  # Nitrogen
  stackedHist(data=list(sim_recovery$N[(sim_recovery$outcome == "recovers-S")],
    ...
    colors=c(colSen, colTol, colRescue, colNS), ylim=c(0, 6),
  ...
  # Prey
  stackedHist(data=list(sim_recovery$X[(sim_recovery$outcome == "recovers-S")],
...
```{r plot-recovery-doc, cache=TRUE, dependson=c("sim-recovery-doc"), fig.width=10, fig.height=5}
  ...
  # Plot the data on the linear discriminant axes
  pdf("bleaching_recovery_doc.pdf", 10, 5)
  layout(matrix(c(1, 1, 2, 3, 1, 1, 4, 5), nrow=2, byrow=TRUE), respect=TRUE)
  ...
  # Light
  stackedHist(data=list(sim_recovery_doc$L[(sim_recovery_doc$outcome == "recovers-S")],
  ...
  points(sim_recovery_doc$L[(sim_recovery_doc$outcome == "recovers-T")],
    rep(6, sum(sim_recovery_doc$outcome == "recovers-T")),col=colTol, pch=pchTol)

  points(sim_recovery_doc$L[(sim_recovery_doc$outcome == "recovers-S-ip-T")],
    rep(5.5, sum(sim_recovery_doc$outcome == "recovers-S-ip-T")),col=colRescue, pch=pchRescue)


  # Nitrogen
  stackedHist(data=list(sim_recovery_doc$N[(sim_recovery_doc$outcome == "recovers-S")],
  ...
  # Prey
  stackedHist(data=list(sim_recovery_doc$X[(sim_recovery_doc$outcome == "recovers-S")],
...
```{r example-plots, cache=TRUE, dependson=c("sim-recovery")}
  ...
  recovers_S_ip_T_ex <- min((1:nrow(sim_recovery))[sim_recovery$outcome == "recovers-S-ip-T"])
  ...
  plot(time, run_recovers_S$S/run_recovers_S$H, type="l", col=colSen, lty=2, lwd=2,
  	xlab="Time (days)", ylab="Symbiont:host ratio",
  	main="Recovery with sen. symb. alone\n('Recovers w/ S.')", ylim=c(0, 1))
  ...
  run_recovers_T <- run_coral_initFluxes(time=time, env=env_recovers_T, pars=pars_tol, initjHG=0, initjCP=0)
  cnlim_recovers_T <- cn_limitation(run_recovers_T, pars_tol)

  plot(time, run_recovers_T$S/run_recovers_T$H, type="l", col=colTol, lty=2, lwd=3,
    xlab="Time (days)", ylab="Symbiont:host ratio",
    main="Recovery with addition of tol. symb\n('Recovers with T.')", ylim=c(0, 1))
  lines(time[(run_recovers_T$dH.Hdt > 0) & (cnlim_recovers_T$H >= 0) & (cnlim_recovers_T$S >= 0)],
    (run_recovers_T$S/run_recovers_T$H)[(run_recovers_T$dH.Hdt > 0) & (cnlim_recovers_T$H >= 0) & (cnlim_recovers_T$S >= 0)],
    lwd=3, col=colTol)
  ...
  plot(time, run_recovers_S_ip_T$S.1/run_recovers_S_ip_T$H, type="l", col=colSen, lty=2, lwd=2,
    xlab="Time (days)", ylab="Symbiont:host ratio",
    main="Recovery to sen-dominated state\nwith addition of tol. symb\n('Recovers w/ S in presence of T')", ylim=c(0, 1))
  ...
  lines(time, run_recovers_S_ip_T$S.2/run_recovers_S_ip_T$H, type="l", col=colTol, lty=2, lwd=3)
  lines(time[(run_recovers_S_ip_T$dH.Hdt > 0) & (cnlim_recovers_S_ip_T$H >= 0) & (cnlim_recovers_S_ip_T$S.1 >= 0) & (cnlim_recovers_S_ip_T$S.2 >= 0)],
  ...
  par(mar=c(0,0,0,0))
  plot.new()
  legend("center", c("Sensitive symb. (recovered)", "Sensitive symb. (pre-recovery)", "Tolerant symb. (recovered)", "Tolerant symb. (pre-recovery)"),
  	col=c(colSen, colSen, colTol, colTol), lwd=c(2, 2, 3, 3), lty=c(1, 2, 1, 2), cex=1.3)
  #dev.off()
