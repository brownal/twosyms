...
What if growth rate is increased?
# Compare seasonal dynamics with either sensitive or tolerant symbiont
```{r incr_gr_s, fig.width=5, fig.height=5}
# Define simulation environment (seasonal fluctuation in light)
time <- seq(1,365,0.1)
env <- init_env(time=time, X=c(2e-7,2e-7,0), N=c(1e-7,1e-7,0), L=c(30,15,3))
...
Tolerant symbiont is lost in about 5 years at constant, intermediate light level.
...
# Define high light-stress environment for simulation
time <- seq(1,3650,0.1)
env <- init_env(time=time, X=c(2e-7,2e-7,0), N=c(1e-7,1e-7,0), L=c(10,10,0))
...
Tolerant symbiont is lost in about 3 years at constant, low light level.