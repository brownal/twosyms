---
title: "Symbiont competitive dynamics - excluding host carbon limitation"
...
```{r run-competition-sims, cache=TRUE, dependson=c("set-pars")}
  ...
  results <- foreach(i=1:nrow(env_params), .combine='rbind', .packages='dplyr') %dopar% {
    ...
    list("L"=env_params[i, "L"], "N"=env_params[i, "N"], "X"=env_params[i, "X"],
      "posGrH"=posGrH, "noClimH"=noClimH, "logTS"=logTS,
  ...
  sim_competition <- transform(sim_competition, L=as.numeric(L), N=as.numeric(N), X=as.numeric(X),
    posGrH=as.logical(posGrH), noClimH=as.logical(noClimH), logTS=as.numeric(logTS),
...
```{r run-tolerant-exclusion-sims, cache=TRUE, dependson=c("set-pars", "run-competition-sims")}
  ...
  results <- foreach(i=1:nrow(env_tol_exclusion), .combine='rbind', .packages='dplyr') %dopar% {
    ...
    list("L"=env_tol_exclusion[i, "L"], "N"=env_tol_exclusion[i, "N"], "X"=env_tol_exclusion[i, "X"],
      ...
      "noClimH"=noClimH, "posGrH100d"=posGrH100d, "noClimH100d"=noClimH100d)
  }
  ...
  sim_tol_exclusion <- transform(sim_tol_exclusion, L=as.numeric(L), N=as.numeric(N), X=as.numeric(X),
    ...
    posGrH100d=as.logical(posGrH100d), noClimH100d=as.logical(noClimH100d))
```
...
```{r plot-symb-competition, cache=TRUE, dependson=c("run-competition-sims"), fig.width=10, fig.height=5}
  ...
  pdf("Clim_competitive_dominance.pdf", 10, 5)
...
```{r plot-light-nitrogen-vs-exclusion, cache=TRUE, dependson=c("run-tolerant-exclusion-sims", "run-sensitive-exclusion-sims")}
  pdf("Clim_competitive_exclusion.pdf", 12, 12)
  layout(matrix(c(1, 2), nrow=2, byrow=TRUE), respect=FALSE)
  ## Light

  plot(sim_tol_exclusion$L[sim_tol_exclusion$reachedThreshold & sim_tol_exclusion$posGrH100d & sim_tol_exclusion$noClimH100d],
  	sim_tol_exclusion$exclusionTime[sim_tol_exclusion$reachedThreshold  & sim_tol_exclusion$posGrH100d & sim_tol_exclusion$noClimH100d]/365,
  ...
  points(sim_sen_exclusion$L[sim_sen_exclusion$reachedThreshold & sim_sen_exclusion$posGrH100d & sim_sen_exclusion$noClimH100d],
  	sim_sen_exclusion$exclusionTime[sim_sen_exclusion$reachedThreshold & sim_sen_exclusion$posGrH100d & sim_sen_exclusion$noClimH100d]/365,
  	col=colSen, pch=pchSen)

  points(sim_tol_exclusion$L[!(sim_tol_exclusion$reachedThreshold) & sim_tol_exclusion$posGrH100d & sim_tol_exclusion$noClimH100d],
  	rep(21, sum(!(sim_tol_exclusion$reachedThreshold) & sim_tol_exclusion$posGrH100d & sim_tol_exclusion$noClimH100d)),
  	col=colTol, pch=17)

  points(sim_sen_exclusion$L[!(sim_sen_exclusion$reachedThreshold) & sim_sen_exclusion$posGrH100d & sim_sen_exclusion$noClimH100d],
  	rep(21, sum(!(sim_sen_exclusion$reachedThreshold) & sim_sen_exclusion$posGrH100d & sim_sen_exclusion$noClimH100d)),
  	col=colSen, pch=2)
  ...
  plot(sim_tol_exclusion$N[sim_tol_exclusion$reachedThreshold & sim_tol_exclusion$posGrH100d & sim_tol_exclusion$noClimH100d],
  	sim_tol_exclusion$exclusionTime[sim_tol_exclusion$reachedThreshold & sim_tol_exclusion$posGrH100d & sim_tol_exclusion$noClimH100d]/365,
  ...
  points(sim_sen_exclusion$N[sim_sen_exclusion$reachedThreshold & sim_sen_exclusion$posGrH100d & sim_sen_exclusion$noClimH100d],
  	sim_sen_exclusion$exclusionTime[sim_sen_exclusion$reachedThreshold & sim_sen_exclusion$posGrH100d & sim_sen_exclusion$noClimH100d]/365,
  	col=colSen, pch=pchSen)

  points(sim_tol_exclusion$N[!(sim_tol_exclusion$reachedThreshold) & sim_tol_exclusion$posGrH100d & sim_tol_exclusion$noClimH100d],
  	rep(21, sum(!(sim_tol_exclusion$reachedThreshold) & sim_tol_exclusion$posGrH100d & sim_tol_exclusion$noClimH100d)),
  	col=colTol, pch=17)
...
```{r plot-food-vs-exclusion, cache=TRUE, dependson=c("run-tolerant-exclusion-sims", "run-sensitive-exclusion-sims")}
  ...
  pdf("Clim_competitive_exclusion_prey.pdf", 6, 6)

  layout(matrix(c(1, 2), nrow=2, byrow=TRUE), respect=FALSE, heights=c(0.75, 0.25))

  plot(sim_tol_exclusion$X[sim_tol_exclusion$reachedThreshold& sim_tol_exclusion$posGrH100d & sim_tol_exclusion$noClimH100d],
  	sim_tol_exclusion$exclusionTime[sim_tol_exclusion$reachedThreshold & sim_tol_exclusion$posGrH100d & sim_tol_exclusion$noClimH100d]/365,
  ...
  points(sim_sen_exclusion$X[sim_sen_exclusion$reachedThreshold & sim_sen_exclusion$posGrH100d & sim_sen_exclusion$noClimH100d],
  	sim_sen_exclusion$exclusionTime[sim_sen_exclusion$reachedThreshold & sim_sen_exclusion$posGrH100d & sim_sen_exclusion$noClimH100d]/365,
  	col=colSen, pch=pchSen)

  points(sim_tol_exclusion$X[!(sim_tol_exclusion$reachedThreshold) & sim_tol_exclusion$posGrH100d& sim_tol_exclusion$noClimH100d],
  	rep(21, sum(!(sim_tol_exclusion$reachedThreshold) & sim_tol_exclusion$posGrH100d & sim_tol_exclusion$noClimH100d)),
  	col=colTol, pch=17)
...
```{r summary-statistics, cache=TRUE, dependson=c("run-tolerant-exclusion-sims", "run-sensitive-exclusion-sims")}
  summary(sim_tol_exclusion[sim_tol_exclusion$posGrH100d, & sim_tol_exclusion$noClimH100d,])

  summary(sim_sen_exclusion[sim_sen_exclusion$posGrH100d & sim_sen_exclusion$noClimH100d,])
```
