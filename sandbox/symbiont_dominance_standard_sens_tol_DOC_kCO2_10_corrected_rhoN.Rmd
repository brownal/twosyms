...
search_params <- data.frame("L"=runif(20000, 5, 40),       # Light, choose values between 5 and 40
                            "logN"=runif(20000, -9, -7),     # DIN, choose values between 0 and 6e-6
                            "X" = runif(20000, 0, 4e-7),     # Food, choose values between 0 and 4e07
                            "kCO2" = rep(10, 20000))   # Host carbon-concentrating ability, choose value of 10
...
results <- foreach(i=1:nrow(search_params), .combine='rbind', .packages='dplyr') %dopar% {
    ...
    env <- coRal::init_env(time=time, L=c(search_params[i, "L"], search_params[i, "L"], 0),
                           N=c(10^search_params[i, "logN"], 10^search_params[i, "logN"], 0), X=c(search_params[i, "X"], search_params[i, "X"], 0))
    ...
    list("L"=search_params[i, "L"], "N"=(10^as.numeric(search_params[i, "logN"])), "X"=search_params[i, "X"],
...
search_params <- expand.grid("L"=seq(5, 40, 0.5),       # Light, choose values between 5 and 40
                            "N"=10^seq(-9, -7, 0.1),     # DIN, choose values between 0 and 6e-6
...
search_params <- data.frame("L"=runif(20000, 5, 40),       # Light, choose values between 5 and 40
                            "N"=10^runif(20000, -9, -7),     # DIN, choose values between 0 and 6e-6
                            "X" = runif(20000, 0, 4e-7),     # Food, choose values between 0 and 4e07
                            "kCO2" = rep(10, 20000))   # Host carbon-concentrating ability, choose value of 10
...
#### What happens if we only look at cases where food > 5e-8?

```{r}
dapc_sims <- no_doc_sims[no_doc_sims$X >= 1e-8, ]
...
#### Check survival of healthy host with each symbiont when food = 0
...
search_params <- data.frame("L"=runif(50000, 5, 40),       # Light, choose values between 5 and 40
                            "N"=runif(50000, 1e-7, 6e-6),     # DIN, choose values between 0 and 6e-6
                            "X" = runif(50000, 0, 2e-7),     # Food, choose values between 0 and 4e07
                            "kCO2" = rep(10, 50000))   # Host carbon-concentrating ability, choose value of 10
...
search_params <- data.frame("L"=runif(100000, 5, 40),       # Light, choose values between 5 and 40
                            "N"=10^runif(100000, -9, -7),     # DIN, choose values between 0 and 6e-6
...
```{r}
rescue_sims <- no_doc_bleached_sims[no_doc_bleached_sims$reason=="Rescue" & !is.na(no_doc_bleached_sims$reason),]
...
for (i in 1:nrow(rescue_sims)) {
  ...
  run <- run_coral_rhoN(time=time, env=env, pars=pars_both)
...
plotted_sims <- rescue_sims[1:9, c("L", "N", "X")]
rownames(plotted_sims) <- c("Sim 1", "Sim 2", "Sim 3", "Sim 4", "Sim 5", "Sim 6", "Sim 7", "Sim 8", "Sim 9")
...
# pars_both$initS[1:2] <- c(0.5, 0.5)
#
# myrun <- run_coral_rhoN(time, env=myenv, pars=pars_both)
#
