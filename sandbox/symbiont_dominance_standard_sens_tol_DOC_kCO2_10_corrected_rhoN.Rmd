...
```{r set-unbleached-pars, cache=TRUE}
...
It looks like 200 days is plenty of time to be able to tell the sign of the host
growth rate and whether there are more tolerant or sensitive symbionts.

```{r test-equil-time, cache=TRUE, dependson=c("set-unbleached-pars")}
# Plot some sample simulations to see how long it takes for simulation
# behavior to stabilize
time<-seq(0, 600, 0.1)
env_set <- expand.grid(L=c(5, 30, 60), N=c(1e-8, 5e-7, 1e-5),
	X=c(0, 2e-7, 4e-7))

par(mfrow=c(4,4), mar=c(5.1, 4.6, 4.1, 4.6))

# Legend
plot.new()
legend("top", c("log(Tol/Cen)", "Host growth rate"),
	col=c("#00FABE", "black"), lwd=c(2, 1), title="Legend")

# Run simulations and plot log(sensitive/tolerant) and host growth rate
for(i in 1:nrow(envs)) {
	env <- init_env(time, L=c(env_set[i, "L"], env_set[i, "L"], 0),
		N=c(env_set[i, "N"], env_set[i, "N"], 0),
		X=c(env_set[i, "X"], env_set[i, "X"], 0))
	run <- run_coral(time, env, pars_both)

	plot(time[-1], (log(run$S.2) - log(run$S.1))[-1], xlab="Time (days)", ylab="log(Tol/Sen)",
		main=paste0("L = ", env_set[i, "L"], ", N = ", env_set[i, "N"], ", X = ", env_set[i, "X"]),
    type="l", col="#00FABE", lwd=2, cex=0.75)

  par(new=TRUE)
  plot(time[-1], run$dH.Hdt[-1], type="l", xaxt = "n", yaxt = "n", ylab = "", xlab = "")
  axis(side = 4)
  mtext("Host growth rate", side = 4, line = 3, cex=0.75)
}

# Add legend again at the end
plot.new()
legend("top", c("log(Tol/Cen)", "Host growth rate"),
	col=c("#00FABE", "black"), lwd=c(2, 1), title="Legend")
...
```{r no-doc-sims, cache=TRUE, dependson=c("set-unbleached-pars")}

# run simulations for 200 days
time<-seq(0, 200, 0.1)

search_params <- data.frame("L"=runif(10000, 5, 60),       # Light, choose values between 5 and 40
...
results <- foreach(i=1:nrow(search_params), .combine='rbind', .packages='dplyr') %dopar% {
    ...
    # Run simulations
    run_sen <- coRal::run_coral(time=time, env=env, pars=pars_sen) # Sensitive symb. alone
    run_tol <- coRal::run_coral(time=time, env=env, pars=pars_tol) # Tolerant symb. alone
    run_both <- coRal::run_coral(time=time, env=env, pars=pars_both) # Sensitive and tolerant symbs


    #### Summary statistics ####
    # Survival: does the coral have positive growth at the end of the simulation?
    ...
    # Dominance: when both symbionts are present, which is more frequent? (test with log(tolerant/sensitive))
    tol_per_sen = log(run_both$S.2[length(time)]) - log(run_both$S.1[length(time)])
...
for (i in 1:nrow(no_doc_sims)) {
      ...
      tol_per_sen <- no_doc_sims[i, "logTS"]
...
mydapc <- dapc(dapc_sims[, c("L", "N", "X")], grp=dapc_sims$dominant_symb, var.contrib=TRUE, var.loadings=TRUE, scale = TRUE, n.pca = 3, n.da = 3)
...
plotStackedHist <- function(data, colors=NULL, legend=NULL, legendLocation=NULL, plotLegend=FALSE, breaks="Sturges", ...) {
  plotargs <- list(...)
...
datalda <- no_doc_sims[, c("dominant_symb", "L", "N", "X")]
...
mylda <- lda(dominant_symb ~ L + N + X, datalda)
ptslda <- predict(mylda, datalda[, c("L", "N", "X")])

pdf("dominant_symb.pdf", 10, 5)
layout(matrix(c(1, 1, 2, 3, 1, 1, 4, 5), nrow=2, byrow=TRUE), respect=TRUE)

plot(predict(mylda)$x[,1], predict(mylda)$x[,2], col=c("#648FFF40", rgb(0, 0, 0, 0.25), "#FFB00040")[no_doc_sims$dominant_symb],
	pch=c(1, 0, 19)[no_doc_sims$dominant_symb], xlab="LD 1", ylab="LD 2", main="(a) Linear discriminant analysis")

arrows(x0=0, y0=0, x1=1.5*mylda$scaling[1,1], y1=1.5*mylda$scaling[1,2], length=0.1, col="white")
text(x=1.25*mylda$scaling[1,1], y=1.25*mylda$scaling[1,2], labels="Light", pos=3, col="white")

arrows(x0=0, y0=0, x1=1.5*mylda$scaling[2,1], y1=1.5*mylda$scaling[2,2], length=0.1, col="black")
text(x=1.25*mylda$scaling[2,1], y=1.25*mylda$scaling[2,2], labels="Nitrogen", pos=2, col="black")

arrows(x0=0, y0=0, x1=1.5*mylda$scaling[3,1], y1=1.5*mylda$scaling[3,2], length=0.1, col="black")
text(x=1.25*mylda$scaling[3,1], y=1.25*mylda$scaling[3,2], labels="Food", pos=2, col="black")


plotStackedHist(data=list(no_doc_sims$L[(no_doc_sims$dominant_symb == "Sensitive")],
  no_doc_sims$L[(no_doc_sims$dominant_symb == "Tolerant")],
  no_doc_sims$L[(no_doc_sims$dominant_symb == "No_survival")]),
  breaks=seq(5, 60, 2.5),
  colors=c("#648FFF", "#FFB000", "black"),
  xlab="Light (mol photons/m^2/day)", ylab="% Simulations", main="(b) Light", xlim=c(5, 60))


plotStackedHist(data=list(no_doc_sims$N[(no_doc_sims$dominant_symb == "Sensitive")],
  no_doc_sims$N[(no_doc_sims$dominant_symb == "Tolerant")],
  no_doc_sims$N[(no_doc_sims$dominant_symb == "No_survival")]),
  colors=c("#648FFF", "#FFB000", "black"),
  xlab="Nitrogen (mol/L)", ylab="% Simulations", main="(c) Nitrogen")

plotStackedHist(data=list(no_doc_sims$X[(no_doc_sims$dominant_symb == "Sensitive")],
  no_doc_sims$X[(no_doc_sims$dominant_symb == "Tolerant")],
  no_doc_sims$X[(no_doc_sims$dominant_symb == "No_survival")]),
  colors=c("#648FFF", "#FFB000", "black"),
...
legend("center", c("Sensitive symb.", "Tolerant symb.", "No survival"),
	col=c("#648FFF", "#FFB000", "black"), pch=c(1, 19, 0),
	title="Legend: Dominant symb.", cex=1.3, pt.cex=1.5)

dev.off()


#predict(mylda, data.frame(L=c(1, 0, 0), N=c(0, 1, 0), X=c(0, 0, 1)))


# allLight <- hist(no_doc_sims$L, breaks=mybreaks, plot=FALSE)
# allLight$counts <- allLight$counts / sum(allLight$counts) * 100 # convert bar heights to %
#
# plot(allLight, freq=FALSE, col="black",
...
### How fast is the non-dominant symbiont lost?

```{r symbiont-loss-barplot, cache=TRUE, dependson=c("no-doc-sims")}

# Convert the log(tol/sen) values to the fraction of symbionts that are the rarer type
# e.g. when the sensitive is the rarer type we convert log(T/S) to S/(S + T)
# exp(log(T/S)) = T/S --> T/S + 1 = (T + S)/S --> ()(T + S)/S)^-1 = S/(S + T)
# (exp(log(T/S)) + 1)^-1 = S/(S + T) and (exp(-1 * log(T/S)) + 1)^-1 = T/(S + T)

fractionS_Tdom <- (exp(no_doc_sims$logTS[no_doc_sims$survival_both &
	(no_doc_sims$dominant_symb == "Tolerant")]) + 1)^-1

fractionT_Sdom <- (exp(-1 * no_doc_sims$logTS[no_doc_sims$survival_both &
	(no_doc_sims$dominant_symb == "Sensitive")]) + 1)^-1

# Use the histogram function to bin these rarer symbiont fractions
histTS_Tdom <- hist(fractionS_Tdom, plot=FALSE, right=FALSE,
	breaks=c(0, 0.0001, 0.001, 0.01, 0.1, 0.5))

histTS_Sdom <- hist(fractionT_Sdom, plot=FALSE, right=FALSE,
	breaks=c(0, 0.0001, 0.001, 0.01, 0.1, 0.5))

# Convert bin heights to % of simulations
histTS_Tdom$density <- histTS_Tdom$counts / sum(histTS_Tdom$counts) * 100
histTS_Sdom$density <- histTS_Sdom$counts / sum(histTS_Sdom$counts) * 100

# Put the bin heights together in a matrix
TS_mat <- matrix(c(histTS_Tdom$density, histTS_Sdom$density), byrow=TRUE, nrow=2)

# Bar plot
barplot(TS_mat,
	names=c("<0.01", "0.01 to 0.1", "0.1 to 1", "1 to 10", "10 to 50"),
	xlab="% of rarer symbiont present at 200 days", ylab="% of simulations",
	beside=TRUE, col=c("#648FFF", "#FFB000"))

# Legend
legend("topright", c("Amount of sensitive symb. (when tol. dominant)",
	"Amount of tolerant symb. (when sen. dominant)"),
	pch=22, pt.bg=c("#648FFF", "#FFB000"), col="black")

# Print results
print(matrix(c("% of symb. that are rare type", "<0.01", "0.01 to 0.1", "0.1 to 1", "1 to 10", "10 to 50",
  "% sensitive symb. (tol. dominant)", histTS_Tdom$density,
  "% tolerant symb. (sen. dominant)", histTS_Sdom$density), byrow=TRUE, nrow=3))
```
...
mydapc <- dapc(dapc_sims[, c("L", "N", "X")], grp=dapc_sims$survival_1symb, var.contrib=TRUE, var.loadings=TRUE, scale = TRUE, n.pca = 3, n.da = 3)
...
```{r dominant-symb-histograms, dev=c("png", "pdf")}
...
```{r bleaching-recovery-dapc, dev=c("png", "pdf")}
...
```{r bleaching-recovery-histograms, dev=c("png", "pdf")}
...
for (i in 1:min(30, nrow(rescue_sims))) {
  ...
  run <- coRal::run_coral_oc(time=time, env=env, pars=pars_both)
...
```{r cache=TRUE, dependson=c("doc-bleached-sims")}
rescue_sims <- doc_bleached_sims[doc_bleached_sims$reason=="Rescue" & !is.na(doc_bleached_sims$reason),]
...
```{r, cache=TRUE, dependson=c("doc-sims")}
pars_both <- def_pars(nsym=2)
pars_both$jCPm[2] <- 1.0
pars_both$kROS[2] <- 250
pars_both$jSGm[2] <- 0.15
