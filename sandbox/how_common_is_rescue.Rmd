...
draws = 30000; # number of sets of random parameter combinations to look over
...
# How does the initial symbiont:host ratio affect the frequency of rescue?
Let's try 10^-4 (we already have this), 10^-3, and 10^-5 to start with
...
draws = 30000; # number of sets of random parameter combinations to look over for each initial S:H ratio
initSHlist = c(1e-3, 1e-4, 1e-5) # initial symbiont to host ratio to look over

search_params <- data.frame(
                            # Environmental parameters
                            "L" = runif(draws*length(initSHlist), 0, 40),       # Light, choose values between 0 and 40
                            "N" = runif(draws*length(initSHlist), 0, 4e-6),     # DIN, choose values between 0 and 4e-6
                            "X" = runif(draws*length(initSHlist), 0, 4e-7),     # Food, choose values between 0 and 4e-7
                            # Host parameter
                            "kCO2" = runif(draws*length(initSHlist), 10, 50),   # Host carbon concentrating, choose values between 10 and 50
                            # Tolerant symbiont parameters
                            "kNPQ" = runif(draws*length(initSHlist), 112, 224), # Non-photochemical quenching, choose values between 112 and 124
                            "astar" = runif(draws*length(initSHlist), 0.67, 1.34), # astar, choose values between 0.7 and 1.34
                            "jSGm" = runif(draws*length(initSHlist), 0.15, 0.25), # Maximum growth rate, choose values between 0.15 and 0.25
                            "jCPm" = runif(draws*length(initSHlist), 1, 2.8), # Maximum photosynthetic rate, choose values between 1 and 2.8
                            "kROS" = runif(draws*length(initSHlist), 80, 250), # kROS, choose values between 80 and 250
                            "b" = runif(draws*length(initSHlist), 1, 5),   # b, choose values between 1 and 5
                            "initSH" = rep(initSHlist, each=draws)) # initial symbiont:host ratio, takes values given in initSHlist
...
results <- foreach(i=1:nrow(search_params), .combine='rbind', .packages='dplyr') %dopar% {
      ...
      pars1$initS <- search_params[i, "initSH"]
    ...
    if (run_1symb$dH.Hdt[length(run_1symb$dH.Hdt)] <= 0) {
         ...
         pars2 <- coRal::def_pars(nsym=2)
         pars2$kCO2 <- search_params[i, "kCO2"]
         pars2$initS[1:2] <- c(0.5, 0.5) * search_params[i, "initSH"]
         pars2$astar[2] <- search_params[i, "astar"]
         pars2$kNPQ[2] <- search_params[i, "kNPQ"]
         pars2$jSGm[2] <- search_params[i, "jSGm"]
         pars2$jCPm[2] <- search_params[i, "jCPm"]
...
# store results in dataframe
rescue_initSH <- data.frame(results, row.names=NULL)

```

## How does the overall prevalence of rescue change?

```{r}
kable(data.frame("initial_S:H"=c(1e-3, 1e-4, 1e-5), "rescue_percent"=c(
      paste0(sum(rescue_initSH[rescue_initSH$initSH == 1e-3, ]$reason=="rescue")/nrow(rescue_initSH[rescue_initSH$initSH == 1e-3, ]) * 100,  "%"),
      paste0(sum(rescue_initSH[rescue_initSH$initSH == 1e-4, ]$reason=="rescue")/nrow(rescue_initSH[rescue_initSH$initSH == 1e-4, ]) * 100,  "%"),
      paste0(sum(rescue_initSH[rescue_initSH$initSH == 1e-5, ]$reason=="rescue")/nrow(rescue_initSH[rescue_initSH$initSH == 1e-5, ]) * 100,  "%"))),
      col.names=c("Initial symbiont:host ratio", "prevalence of rescue (% of simulations)"))
```

```{r}
par(mfrow=c(2,2))

for (sh in c(1e-3, 1e-4, 1e-5)) {
      initSHsims <- rescue_initSH[rescue_initSH$initSH == sh, ]

      plot(initSHsims[(initSHsims$reason == "S_lives"), "L"], initSHsims[(initSHsims$reason == "S_lives"), "X"],
             pch=1, col="#648FFF80",
             xlab=expression(paste("Light (mol photons ", "m"^"-2", "d"^"-1", ")")),
             ylab="Food (C-mol)",
             main=paste0("Initial S:H ratio = ", sh),
             xlim=c(0, 40), ylim=c(0e-7, 4e-7))

       points(initSHsims[(initSHsims$reason == "T_dies"), "L"], initSHsims[(initSHsims$reason == "T_dies"), "X"],
             pch=0, col=rgb(0, 0, 0, alpha=0.4))
       points(initSHsims[(initSHsims$reason == "T_dominates"), "L"], initSHsims[(initSHsims$reason == "T_dominates"), "X"],
             pch=19, col="#FFB000")
      points(initSHsims[(initSHsims$reason == "rescue"), "L"], initSHsims[(initSHsims$reason == "rescue"), "X"],
            pch=15, col="#DC267F")
}

      plot(NULL, xaxt='n',yaxt='n',bty='n',ylab='',xlab='', xlim=0:1, ylim=0:1)
      legend("top", c("No recovery", "Recovers w/ sensitive symbiont", "Recovers w/ tolerant symbiont", "Recovers w/ sens. in presence of tol."),
            col=c(rgb(0, 0, 0), "#648FFF", "#FFB000", "#DC267F"), pch=c(0, 1, 19, 15), bg="white")
```

## How does the prevalence of rescue in a region where it's generally "high" change?
Light 20 to 30 mol photons/(m^2 * d) and food 1e-7 to 2e-7 C-mol

```{r}

sims_in_region <- rescue_initSH[(rescue_initSH$L >= 20) & (rescue_initSH$L <= 30) & (rescue_initSH$X >= 1e-7) & (rescue_initSH$X <= 2e-7), ]

kable(data.frame("initial_S:H"=c(1e-3, 1e-4, 1e-5), "rescue_percent"=c(
      paste0(sum(sims_in_region[sims_in_region$initSH == 1e-3, ]$reason=="rescue")/nrow(sims_in_region[sims_in_region$initSH == 1e-3, ]) * 100,  "%"),
      paste0(sum(sims_in_region[sims_in_region$initSH == 1e-4, ]$reason=="rescue")/nrow(sims_in_region[sims_in_region$initSH == 1e-4, ]) * 100,  "%"),
      paste0(sum(sims_in_region[sims_in_region$initSH == 1e-5, ]$reason=="rescue")/nrow(sims_in_region[sims_in_region$initSH == 1e-5, ]) * 100,  "%"))),
      col.names=c("Initial symbiont:host ratio", "prevalence of rescue (% of simulations)"))
```
...
for (sh in c(1e-3, 1e-4, 1e-5)) {
      initSHsims <- sims_in_region[sims_in_region$initSH == sh, ]
