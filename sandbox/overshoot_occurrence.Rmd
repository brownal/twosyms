...
S_equil <- foreach(i=1:nrow(S_lives_cases), .combine='rbind', .packages='dplyr') %dopar% {
      ...
      pars2 <- coRal::def_pars(nsym=2)
      pars2$kCO2 <- as.numeric(S_lives_cases[i, "kCO2"])
      pars2$initS[1:2] <- c(0.5e-4, 0.5e-4)
      pars2$astar[2] <- as.numeric(S_lives_cases[i, "astar"])
      pars2$kNPQ[2] <- as.numeric(S_lives_cases[i, "kNPQ"])
      pars2$jSGm[2] <- as.numeric(S_lives_cases[i, "jSGm"])
      pars2$jCPm[2] <- as.numeric(S_lives_cases[i, "jCPm"])
      pars2$kROS[2] <- as.numeric(S_lives_cases[i, "kROS"])
      pars2$b[2] <- as.numeric(S_lives_cases[i, "b"])
...
## What was the maximum S:H host ratio reached in the first 100 days (maximum potential overshoot)?
```{r}
par(mfrow=c(3,2))
hist(as.numeric(rescue_tol_param$max_SH), main="All outcomes", xlab="Maximum symbiont:host ratio (first 100 days)", ylab="Number of simulations")

hist(as.numeric(rescue_tol_param[rescue_tol_param$reason=="S_lives", "max_SH"]),
      main="Recovers w/ sensitive symb. alone", xlab="Maximum symbiont:host ratio (first 100 days)", ylab="Number of simulations")
hist(as.numeric(rescue_tol_param[rescue_tol_param$reason=="rescue", "max_SH"]),
      main="Recovers w/ sensitive symb. when tolerant symb. is present", xlab="Maximum symbiont:host ratio (first 100 days)", ylab="Number of simulations")
hist(as.numeric(rescue_tol_param[rescue_tol_param$reason=="T_dominates", "max_SH"]),
      main="Recovers w/ tolerant symb.", xlab="Maximum symbiont:host ratio (first 100 days)", ylab="Number of simulations")
hist(as.numeric(rescue_tol_param[rescue_tol_param$reason=="T_dies", "max_SH"]),
      main="Does not recover", xlab="Maximum symbiont:host ratio (first 100 days)", ylab="Number of simulations")
```

## When the host recovers, what is the maximum S:H ratio ever reached (accounts for overshoot at equilibrating at a high S:H ratio)?
