---
title: "How much tolerant is required for survival?"
...
```{r plot-results-heatmap, cache=TRUE, depends=c("run-sims")}
#pdf("Tolerant_amount_vs_survival_light.pdf", 15, 7.5)
...
plot(lightMat[nrow(lightMat):1,]*100, breaks=c(0, 1/(nrow(sim_ts_reqd) + 1), seq(0.2, 0.8, 0.2), 1 - (1/(nrow(sim_ts_reqd) + 1)), 1)*100,
  ...
  main="Effect of amount of tolerant symbiont on host survival", asp=TRUE, border=NA, key=NULL)
...
#pdf("Tolerant_amount_vs_survival_nitrogen_prey.pdf", 15, 15)
layout(matrix(c(1, 3, 2, 3), nrow=2, byrow=TRUE), width=c(5, 1))
...
plot(nitrogenMat[nrow(nitrogenMat):1,], breaks=c(0, 1/(nrow(sim_ts_reqd) + 1), seq(0.2, 0.8, 0.2), 1 - (1/(nrow(sim_ts_reqd) + 1)), 1),
  ...
  main="(a) Nitrogen", asp=TRUE, border=NA, key=NULL)
...
plot(foodMat[nrow(foodMat):1,], breaks=c(0, 1/(nrow(sim_ts_reqd) + 1), seq(0.2, 0.8, 0.2), 1 - (1/(nrow(sim_ts_reqd) + 1)), 1),
  ...
  main="(b) Prey", asp=TRUE, border=NA, key=NULL)
