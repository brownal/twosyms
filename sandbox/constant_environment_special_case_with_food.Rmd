...
```{r}
inputDF <- expand.grid("L"=seq(0, 30, 3), "N"=seq(1e-7, 9e-7, 2e-7), "X"=c(5e-8, 1e-7, 1.5e-7))
...
oneSymbOutput <- foreach(i=1:nrow(inputDF), .combine='rbind', .packages='dplyr') %dopar% {
    ...
    list("L"=inputDF[i, "L"], "N"=inputDF[i, "N"], "X"=inputDF[i, "X"],
...
```{r}
par(mfrow=c(2, 2), pty="s", mai=c(1,1,0.25,0.25))


for (X in c(5e-8, 1e-7, 1.5e-7)) {
...
for (X in c(5e-8, 1e-7, 1.5e-7)) {
  ...
  colmax = max(as.numeric(twoSymbs$gr))
  colbreak = colmax/10
  plot(mat2, xlab="N", ylab="L", main=paste0("X = ", X), na.col="lightgray", na.print=FALSE,
       breaks=seq(0, colmax, colbreak), col=viridis(10),
       key=NULL, fmt.cell='%.2f')
}
...
for (X in c(5e-8, 1e-7, 1.5e-7)) {
  data <- oneSymb$last_sign_change_gr[(oneSymb$X==X)]
  mat <- matrix(as.numeric(data), nrow=11, dimnames = list(unique(oneSymb$L), unique(oneSymb$N)))
  mat <- mat[nrow(mat):1,]
...
for (X in c(5e-8, 1e-7, 1.5e-7)) {
  data <- twoSymbs$last_sign_change_gr[(twoSymbs$X==X)]
  mat <- matrix(as.numeric(data), nrow=11, dimnames = list(unique(twoSymbs$L), unique(twoSymbs$N)))
  mat <- mat[nrow(mat):1,]
...
for (X in c(5e-8, 1e-7, 1.5e-7)) {
  ...
  st <- log10(as.numeric(twoSymbs$finalST[(twoSymbs$X==X)]))
