...
```{r}
rescue_for_pca <- with(rescue_tol_param, {data.frame(rescue=as.numeric(rescue), L=as.numeric(L), N=as.numeric(N), astar=as.numeric(astar),
      kNPQ=as.numeric(kNPQ), jSGm=as.numeric(jSGm), jCPm=as.numeric(jCPm), kROS=as.numeric(kROS), b=as.numeric(b))})
#pca <- prcomp(~ L + N + astar + kNPQ + jSGm + jCPm + kROS + b, data=rescue_for_pca, scale=TRUE, center=TRUE)
# print(pca)
# summary(pca)
# mycolors <- as.factor(rescue_for_pca$rescue)
# fviz_pca_ind(pca, label="none", alpha.ind=(0.25*rescue_for_pca$rescue+0.05), col.ind=mycolors, palette=c("gray", "blue"), legend.title="Rescue (1 =TRUE)", addEllipse=TRUE)
# fviz_pca_var(pca)
mysvm <- svm(rescue_for_pca[,names(rescue_for_pca)!="rescue"], rescue_for_pca$rescue)
print(mysvm)
summary(mysvm)
```
