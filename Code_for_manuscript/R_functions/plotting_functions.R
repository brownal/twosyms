...
# A function for plotting the distribution of different categories vs a dependent variable
# This function basically makes a stacked histogram and then normalizes the heights of the histogram
# bars so they are all the same. This is really useful for showing how the frequency of categories changes
# as a variable changes. BUT it can also be misleading if you are trying to get a regular stacked histogram,
# as it'll get rid of any variation in bar height. USE WITH CAUTION!!!
# data should be a list of the form (data1=data to plot 1st, data2=data to plot 2nd, ...)
evenBarsHist <- function(data, colors=NULL, legend=NULL, legendLocation=NULL, plotLegend=FALSE, breaks="Sturges", ...) {
  plotargs <- list(...)
  ...
  histAll <- hist(unlist(data), breaks=breaks, plot=FALSE)
  breaks <- histAll$breaks # get the breaks to use for the other histograms
  normalizeBy <- histAll$counts # Get the total number of simulations in each bin to normalize by
