...
#' @param initjHG the initial host growth rate
#' @param initjCP the initial symbiont photosynthetic rate(s).
#'  Enter as a list for multiple symbionts.
...
run_coral_oc_initFluxes <- function(time, env, pars, initjHG, initjCP) {
  ...
  # Set up the vectors for symbiont fluxes
  for (x in c("rNS", "jL", "jCP", "jeL", "jNPQ", "jSG", "rhoC", "jST", "rCS", "cROS", "dS.Sdt", "S")) {
    assign(x, matrix(NA, ncol=nsym, nrow=length(time)))
  }

  # Initialize environmental fluxes (only depend on environment and pars)
  jX <- (pars$jXm * env$X / (env$X + pars$KX))
  jN <- (pars$jNm * env$N / (env$N + pars$KN))
  jL[1,] <- env$L[1] * pars$astar
  jOC <- rep(0.0167, length(time))  # HOST ORGANIC CARBON UPTAKE FROM ENVIRONMENT

  # Initialize constant fluxes (only depend on pars)
  ...
  # Initialize jHG and jCP
  jHG[1] <- initjHG
  jCP[1,] <- initjCP

  # Initialize fluxes that depend on other fluxes
  rhoN[1] <- max(jN[1] + pars$nNX * jX[1] + rNH[1] - pars$nNH * jHG[1], 0)
  jeL[1,] <- pmax(jL[1,] - jCP[1,]/pars$yCL, 0)
  rCH[1] <- pars$sigmaCH * (pars$jHT0 + (1-pars$yC)*jHG[1]/pars$yC)

  ## Requires rhoN
  jSG[1,] <- vsynth(pars$yC*jCP[1,], (rhoN[1]*H[1]/sum(S[1,]) + rNS[1,])/pars$nNS, pars$jSGm)

  ### Requires jSG
  rCS[1,] <- pars$sigmaCS * (pars$jST0 + (1-pars$yC)*jSG[1,]/pars$yC)
  rhoC[1,] <- pmax(jCP[1,] - jSG[1,]/pars$yC, 0)

  #### Requires rhoC
  jeC[1] <- max(jX[1] + jOC[1] + sum(rhoC[1,]*S[1,])/H[1] - jHG[1]/pars$yC, 0)
  ...
  ### Requires jeL and jNPQ
  cROS[1,] <- 1 + (pmax(jeL[1,] - jNPQ[1,], 0) / pars$kROS)^pars$k

  #### Requires cROS
  jST[1,] <- pars$jST0 * (1 + pars$b * (cROS[1,] - 1))
